...
<template>
<div>
    <div v-for="(task, index) in tasks" :key="task.id" class="w-1/4">
        <div class="h-20 drop-shadow-sm rounded-md bg-white flex items-center justify-between m-4">
            ...
            <div v-if="task.completed" class="h-[72px] w-[4px]" style="background: url(src/assets/undone.svg)"></div>
            <div v-else class="h-[72px] w-[4px]" style="background: url(src/assets/done.svg)"></div>
        </div>
    </div>
</div>
</template>
